...
Durante la realización de este Trabajo Final de Máster se han utilizado herramientas de inteligencia artificial generativa como apoyo auxiliar en distintas fases del proyecto. En particular, se ha empleado ChatGPT para tareas de ideación, estructuración de apartados, mejora de redacción, revisión de coherencia narrativa, explicación de conceptos técnicos y apoyo en la depuración o reorganización de fragmentos de código.

El uso de estas herramientas se ha limitado a funciones de asistencia. Las decisiones metodológicas, la selección de datos, la ejecución de los notebooks, el entrenamiento de modelos, la generación de resultados, la interpretación final y la validación de los contenidos han sido responsabilidad del autor. Los resultados numéricos, tablas y figuras incluidos en la memoria proceden de la ejecución del código desarrollado para el proyecto y no de contenidos generados automáticamente por herramientas de IA.

Los textos generados o sugeridos mediante inteligencia artificial han sido revisados, modificados y validados antes de su incorporación a la memoria. En particular, se ha comprobado que las explicaciones fueran coherentes con la metodología realmente aplicada, que no se atribuyeran capacidades no implementadas al sistema desarrollado y que las limitaciones del trabajo quedaran reflejadas de forma explícita.

Asimismo, se ha evitado utilizar la inteligencia artificial generativa como fuente bibliográfica primaria. Las referencias científicas empleadas en la memoria proceden de fuentes documentales consultadas y citadas de forma independiente. La IA generativa se ha utilizado como herramienta de apoyo a la escritura y organización del trabajo, no como sustituto del análisis crítico ni de la validación técnica.

En consecuencia, la autoría intelectual del trabajo, la responsabilidad sobre los resultados presentados y la validación final de la memoria recaen íntegramente en el autor. El uso de herramientas de inteligencia artificial generativa se ha realizado como apoyo complementario dentro del proceso de elaboración del TFM.
...
Este Trabajo Final de Máster ha desarrollado una \textit{pipeline} computacional para la generación, evaluación y priorización de moléculas \textit{drug-like} con señal \textit{proxy} de actividad frente a EGFR. El objetivo principal no era validar experimentalmente nuevos inhibidores, sino comprobar si la combinación de un modelo generativo molecular preentrenado, un modelo QSAR de actividad, predicciones ADMET y una estrategia de selección multiobjetivo podía transformar una muestra generativa amplia en un conjunto reducido de hipótesis moleculares priorizadas bajo criterios explícitos.

Los resultados obtenidos muestran que la estrategia propuesta es viable como herramienta de priorización temprana. A partir de 10.000 SMILES brutos generados, la \textit{pipeline} obtuvo 9.068 moléculas válidas, únicas y nuevas respecto al conjunto de entrenamiento. Tras aplicar filtros fisicoquímicos, predicción de actividad, evaluación ADMET, control de similitud estructural y selección mediante dominancia de Pareto, el conjunto se redujo hasta una \textit{shortlist} final de 30 candidatas. Esta reducción progresiva demuestra que la calidad final de las candidatas no procede únicamente del modelo generativo, sino del proceso posterior de evaluación y selección multiobjetivo.

El modelo generativo ajustado al dominio químico asociado a EGFR mantuvo una calidad generativa adecuada. La época 3 fue seleccionada como punto de compromiso entre adaptación al dominio y conservación de validez, unicidad y novedad. Esta decisión confirma que, en modelos generativos moleculares, continuar el ajuste durante más épocas no implica necesariamente una mejora de la calidad final, ya que puede aumentar el riesgo de pérdida de validez o de acercamiento excesivo al conjunto de entrenamiento.

El modelo QSAR empleado proporcionó una señal útil de priorización relativa, aunque no debe interpretarse como una medida exacta de potencia biológica. Su rendimiento fue razonable para una tarea de cribado temprano, especialmente por la correlación de Spearman observada en el conjunto de prueba. Sin embargo, el error absoluto medio obtenido obliga a interpretar las predicciones de pIC$_{50}$ como una aproximación y no como evidencia directa de afinidad experimental. Esta limitación es especialmente importante en las moléculas más alejadas del dominio químico del conjunto de entrenamiento.

La selección multiobjetivo permitió identificar moléculas con distintos perfiles de compromiso. La \textit{shortlist} final no debe entenderse como un ranking lineal de mejores compuestos, sino como una cartera de hipótesis con perfiles diferenciados. Algunas candidatas destacan por su equilibrio global entre actividad predicha, calidad molecular y criterios ADMET; otras presentan mayor actividad estimada pero también señales de riesgo; y otras resultan interesantes por su novedad estructural frente a inhibidores EGFR aprobados o de referencia. Esta diversidad de perfiles es precisamente una de las ventajas del enfoque basado en Pareto, ya que evita reducir el problema a una única función de recompensa agregada.

Uno de los resultados más relevantes del trabajo es que la actividad predicha por sí sola no es suficiente para seleccionar candidatas defendibles. Aunque una parte importante de las moléculas generadas presentó valores favorables de pIC$_{50}$ predicha, muchas de ellas mostraron riesgos ADMET, baja calidad molecular relativa o incertidumbre por similitud estructural. En particular, el criterio hERG apareció como una de las principales señales de riesgo dentro del conjunto final. Esto refuerza la necesidad de incorporar restricciones farmacocinéticas y toxicológicas desde fases tempranas del diseño molecular computacional.

Desde el punto de vista metodológico, el trabajo confirma que una estrategia de optimización post-generación puede ser útil cuando el modelo generativo empleado no es condicional. En este caso, el modelo MRL genera moléculas a partir de una distribución aprendida, mientras que la optimización se realiza posteriormente mediante scoring, filtrado, control de dominio de aplicabilidad y selección multiobjetivo. Esta aproximación es menos ambiciosa que un modelo generativo directamente condicionado por objetivos, pero resulta más sencilla, reproducible y adecuada para una arquitectura modular de ciencia de datos aplicada.

En conjunto, los resultados son coherentes con lo esperado: el modelo generativo produce muchas moléculas químicamente válidas y razonables, pero solo una fracción reducida mantiene simultáneamente actividad estimada, perfil ADMET aceptable, novedad estructural y calidad molecular. El resultado no debe interpretarse como el descubrimiento de nuevos inhibidores de EGFR, sino como una demostración de que la \textit{pipeline} permite priorizar de forma trazable y cuantitativa un subconjunto manejable de moléculas para análisis posteriores.

\section{Grado de cumplimiento de los objetivos}

El objetivo principal del trabajo se considera alcanzado. Se ha diseñado e implementado una \textit{pipeline} computacional modular que integra generación molecular, evaluación QSAR, predicción ADMET, cálculo de descriptores fisicoquímicos, análisis de similitud estructural y selección mediante dominancia de Pareto.

También se han cumplido los objetivos relacionados con la generación de estructuras SMILES válidas, la integración de un modelo predictivo de actividad frente a EGFR, la incorporación de criterios ADMET y la construcción de una \textit{shortlist} final de candidatas priorizadas. Asimismo, se ha analizado la novedad estructural y la diversidad química del conjunto final mediante similitud de Tanimoto y scaffolds de Murcko.

No obstante, algunos objetivos iniciales deben interpretarse con matices. La actividad frente a EGFR se ha estimado mediante un modelo QSAR y, por tanto, representa una señal \textit{proxy}. Además, la optimización no se ha realizado durante la generación molecular, sino en una fase posterior de evaluación y selección. Por ello, el trabajo demuestra la utilidad de una estrategia de priorización post-generación, pero no la de un modelo generativo condicionado directamente por múltiples objetivos.

\section{Seguimiento de la planificación y metodología}

La metodología seguida fue adecuada para el alcance del trabajo, ya que permitió dividir el problema en módulos independientes y reproducibles: preprocesado, generación, scoring, selección y análisis final. Esta separación facilitó la trazabilidad de los resultados y permitió auditar cada etapa de la \textit{pipeline}.

Durante el desarrollo fue necesario introducir decisiones metodológicas específicas para garantizar la calidad del flujo. Entre ellas destacan la selección de un checkpoint intermedio del modelo generativo, la evaluación del modelo QSAR mediante partición por scaffold, la definición de un conjunto elegible antes de aplicar Pareto y la separación entre filtros ADMET duros y puntuaciones ADMET blandas. Estas decisiones mejoraron la robustez del análisis y evitaron una interpretación excesivamente optimista de las moléculas generadas.

La principal limitación metodológica es que todos los resultados dependen de modelos computacionales. Ni la actividad predicha ni las propiedades ADMET sustituyen a ensayos experimentales. Además, la ausencia de validación estructural mediante \textit{docking} o dinámica molecular limita la capacidad de evaluar la plausibilidad de unión de las candidatas al bolsillo de EGFR. Por tanto, la \textit{pipeline} debe entenderse como una herramienta de cribado y priorización temprana, no como una validación farmacológica.

\section{Impacto ético, social y de sostenibilidad}

El trabajo contribuye positivamente a la sostenibilidad en la medida en que propone una estrategia computacional para priorizar moléculas antes de una posible síntesis o validación experimental. Este tipo de aproximación puede ayudar a reducir recursos materiales, tiempo y costes en fases tempranas del descubrimiento de fármacos.

Al mismo tiempo, el uso de modelos preentrenados y de experimentos controlados permitió limitar el coste computacional del proyecto, evitando entrenamientos profundos desde cero. No obstante, se reconoce que cualquier flujo basado en inteligencia artificial implica consumo energético y que este aspecto debe considerarse en futuros escalados de la metodología.

Desde el punto de vista ético, el principal riesgo sería sobreinterpretar las moléculas generadas como candidatos farmacológicos reales. Para mitigar este riesgo, el trabajo presenta de forma explícita sus limitaciones: las predicciones QSAR y ADMET son aproximaciones estadísticas, las candidatas no han sido validadas experimentalmente y los resultados deben considerarse hipótesis para análisis posterior. Esta comunicación transparente es esencial en aplicaciones de inteligencia artificial al descubrimiento de fármacos.

\section{Trabajos futuros}

Como continuación natural del trabajo, la primera línea futura sería incorporar validación estructural mediante \textit{docking} molecular frente a estructuras relevantes de EGFR, incluyendo variantes asociadas a resistencia cuando sea posible. Esto permitiría evaluar si las moléculas priorizadas presentan modos de unión plausibles y complementar la señal QSAR con información estructural.

Una segunda línea consistiría en refinar el tratamiento de riesgos ADMET, especialmente hERG. Dado que varias candidatas finales presentan señales desfavorables en este endpoint, sería conveniente construir una \textit{shortlist} conservadora alternativa que exija superar hERG como criterio obligatorio, y compararla con la \textit{shortlist} actual.

También sería recomendable incorporar estimaciones adicionales de sintetizabilidad y disponibilidad química, como SA score, retrosíntesis computacional o búsqueda de fragmentos comerciales. Esto permitiría distinguir mejor entre moléculas interesantes desde el punto de vista computacional y moléculas realmente viables para validación experimental.

Otra mejora futura sería comparar de forma más sistemática la selección multiobjetivo frente a baselines alternativos, como ranking exclusivo por pIC$_{50}$, ranking por \textit{reward score} agregado o muestreo generativo sin selección Pareto. Esta comparación permitiría cuantificar con mayor precisión el valor añadido de la estrategia multiobjetivo.

Finalmente, una línea más ambiciosa sería sustituir la optimización post-generación por un modelo generativo condicionado o por un esquema de aprendizaje por refuerzo que incorpore actividad, ADMET, novedad y sintetizabilidad durante la propia generación. Esto permitiría estudiar si la optimización integrada produce mejores perfiles que la estrategia modular desarrollada en este trabajo.
